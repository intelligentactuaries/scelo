// Theme handler — ported from website_v2/src/lib/theme.ts so swarms and
// the public site share storage semantics. Default is "system" (no
// localStorage entry), with explicit "light" / "dark" choices persisted
// under the same key the website uses (`ia.theme`) — so a user who's
// set dark mode on the website lands in dark mode here too.
