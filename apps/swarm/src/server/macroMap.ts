...
export const SA_MACRO_PROVENANCE: string[] = [
  ...
  'Severity is self-reported by each persona and then FLOORED by a clinical risk band computed from its own age and comorbidities (very high → severe, high → moderate, moderate → mild). The floor only ever raises: a persona reporting a worse course keeps it. Personas systematically under-call their own prognosis — on a measured run every agent said asymptomatic or mild, including an immunosuppressed diabetic — which zeroed deaths, admissions and severe cases on every scenario, since all three are coupled to severity.',
  'The floor releases the model\'s own hospitalisation and mortality figures for high-risk agents rather than supplying new ones: no fatality rate is invented here, and an admission the persona refused is not manufactured.',
