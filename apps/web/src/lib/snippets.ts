...
const PYTHON_SNIPPETS: Snippet[] = [
  ...
  {
    prefix: "scelo-lifelib-basicterm",
    detail: "lifelib BasicTerm_ME on your model points (create → read_model → result_pv)",
    body: [
      "import os",
      "import lifelib",
      "import modelx as mx",
      "import pandas as pd",
      "",
      "# lifelib is a library of modelx models: copy the library once, then read a model from it.",
      'LIB = "${1:basiclife}"',
      "if not os.path.isdir(LIB):",
      "    lifelib.create(LIB, LIB)",
      'm = mx.read_model(os.path.join(LIB, "${2:BasicTerm_ME}"))',
      "P = m.Projection",
      "",
      "# policy_id index; age_at_entry, sex, policy_term, policy_count, sum_assured, duration_mth",
      'mp = pd.read_csv("${3:model_points.csv}").set_index("policy_id")',
      "P.model_point_table = mp",
      "",
      "pv = P.result_pv()      # per-policy PV premiums / claims / expenses / commissions / net cash flow",
      "cf = P.result_cf()      # aggregate monthly cash flows",
      '${0:print(pv["PV Net Cashflow"].sum())}',
    ].join("\n"),
  },
  {
