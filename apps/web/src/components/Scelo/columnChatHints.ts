...
export function buildColumnStageContext(meta: ColumnMeta): string {
  ...
  return [
    ...
    "SCOPE RULE for imputation and outliers: `impute-missing` / `cap-outliers` in a clean block treat EVERY flagged column in the dataset (median/mode fill with `was_missing_…` indicators; Tukey-fence clamp). When the user says to impute or cap THIS column only, emit a `transform` instead: `coalesce(value, median(value))` fills numeric nulls, `coalesce(value, mode(value))` fills categorical nulls, and `min(max(value, <loFence>), <hiFence>)` caps at the fences shown in THIS COLUMN below. Both learn from all rows — re-fit on the training split before modelling.",
    "",
    ...
    "",
    "User: fill the missing values here",
    "Reply:",
    "```transform",
    `{"column": "${meta.name}", "formula": "coalesce(value, ${meta.type === "number" ? "median(value)" : "mode(value)"})"}`,
    "```",
    "The fill is learned from all rows — re-fit on the training split before modelling. Run `impute-missing` in a clean block instead if the whole dataset needs it (that path also adds was_missing flags).",
    "",
    ...capExample,
